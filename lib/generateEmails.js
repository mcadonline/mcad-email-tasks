...
import getDirnameFromImportMeta from './getDirnameFromImportMeta.js';
...
async function generateEmails({ template, records, to, from, bcc, requiredFields = [] }) {
  const templatePath = join(__dirname, `../tasks/${template}`);
  ...
  const [completeRecords, incompleteRecords] = partition(hasRequiredFields, records);
