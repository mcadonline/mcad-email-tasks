...
const makeMessageMandrilFriendly = ({ to, from, subject, html, text, bcc }) => {
  ...
  // lets dedupe here, Since we use transactional emails now we dont want to send the same message to
  // the same mail address. Therefore, we need to dedupe the list before sending the email.
  const emailList = {};
  const dedupedToEmails = toEmails.reduce((acc, value) => {
    if (!emailList[value.email]) {
      acc.push(value);
      emailList[value.email] = true;
    }
